...
31. **YAML `permissions: pull-requests: write` does NOT override the repo-policy `Allow GitHub Actions to create and approve pull requests` toggle** (Settings → Actions → General → Workflow permissions; default off org-wide since 2022). When the repo-policy says no, `gh pr create` from `github-actions[bot]` exits 1 with GraphQL `GitHub Actions is not permitted to create or approve pull requests (createPullRequest)`. Branch+commit are pushed successfully *before* the failure. Pattern: any workflow that opens PRs should fail-soft on this exact error (grep the stderr) and emit a `::notice::` with the compare URL — see `.github/workflows/jmh-baseline-bootstrap.yml`. The maintainer can either flip the repo policy or click the URL; YAML alone cannot fix it.
