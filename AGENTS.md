...
29. **`gradle/verification-metadata.xml` regen MUST run with `-Pfluxo.dogfood=false` AND include `cyclonedxBom` in the task list.** Off-CI default is composite mode (gotcha #11/invariant I1), which resolves the harness from the sibling filesystem and never touches Plugin Portal — so the published `io.github.fluxo-kt.fluxo-kmp-conf` JAR + every transitive POM stay unpinned and CI explodes with `DependencyVerificationException` on every job that loads the buildscript. The `cyclonedxBom` task pulls in the SBOM's transitive-POM closure that `help build apiCheck` alone doesn't enumerate. Canonical command in `RELEASING.md`; auto-regen workflow at `.github/workflows/verify-metadata.yml` (Dependabot-only — humans regen locally with both flags).
