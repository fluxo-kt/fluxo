...
29. **`gradle/verification-metadata.xml` regen MUST include `cyclonedxBom` in the task list.** The CycloneDX plugin resolves transitive POMs that `help build apiCheck` alone doesn't enumerate — omit it and those POMs stay unpinned, breaking the release-time SBOM step under strict verification. Canonical command in `RELEASING.md`; auto-regen workflow at `.github/workflows/verify-metadata.yml`.
